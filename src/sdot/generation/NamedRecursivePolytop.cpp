...
void NamedRecursivePolytop::write_primitive_shape_impl( std::ostream &os, GlobGeneGeomData &gggd, const std::vector<NamedRecursivePolytop> &available_primitive_shapes ) const {
    ...
    os << "    virtual void        cut_count  ( const std::function<void(const ShapeType *,BI)> &fc, const BI *case_offsets ) const override;\n";
    ...
    // cut count
    os << "void " << name << "::cut_count( const std::function<void(const ShapeType *,BI)> &fc, const BI *case_offsets ) const {\n";
    os << "    fc( this,\n";
    for( std::uint64_t n = 0; n < cut_cases.size(); ++n )
        os << "        ( case_offsets[ " << n + 1 << " ] - case_offsets[ " << n << " ] ) * " << cut_cases[ n ].nb_created( name ) << ( n + 1 < cut_cases.size() ? " +" : "" ) << "\n";
    os << "    );\n";
    os << "}\n";
    os << "\n";
    os << "\n";
