...
void GlobGeneGeomData::write_gen_def_mk_item( std::ostream &os, bool /*gpu*/, const CutOp &cut_op ) {
    ...
    for( TI no = 0; no < cut_op.cut_items.size(); ++no )
        for( TI nn = 0; nn < cut_op.cut_items[ no ].faces.size(); ++nn )
            if ( cut_op.cut_items[ no ].faces[ nn ] == TI( CutItem::cut_face ) )
