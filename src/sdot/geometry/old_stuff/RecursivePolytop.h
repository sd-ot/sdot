...
public:
    ...
    template<class Pts> auto      with_points          ( const Pts &pts ) const -> RecursivePolytop<typename std::decay<decltype( pts[ 0 ][ 0 ] )>::type,dim,TI,UserData>; ///< normal can be updated correctly only if convex
...
private:
    ...
    mutable TI                    date;                ///< date for graph operations
