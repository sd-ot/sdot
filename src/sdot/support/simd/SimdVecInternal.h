...
#define SIMD_VEC_IMPL_REG_LOAD_ALIGNED( COND, T, SIZE, FUNC ) \
    template<class Arch> \
    typename std::enable_if<COND,Impl<T,SIZE,Arch>>::type load_aligned( const T *data, S<Impl<T,SIZE,Arch>> s ) { \
        Impl<T,SIZE,Arch> res; res.data.reg = FUNC; return res; \
    }

#define SIMD_VEC_IMPL_REG_LOAD_ALIGNED_FOT( COND, T, G, SIZE, FUNC ) \
    template<class Arch> \
    typename std::enable_if<COND,Impl<T,SIZE,Arch>>::type load_aligned( const G *data, S<Impl<T,SIZE,Arch>> s ) { \
        Impl<T,SIZE,Arch> res; res.data.reg = FUNC; return res; \
    }
