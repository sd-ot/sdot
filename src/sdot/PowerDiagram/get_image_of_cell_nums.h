...
void get_image_of_cell_nums( std::size_t *res, Grid &grid, Bounds &bounds, const Pt *positions, const TF *weights, std::size_t nb_diracs, const Func &radial_func, Pt min_pt, Pt max_pt, std::array<std::size_t,Grid::dim> nbp ) {
    ...
    for( std::size_t d = 1; d < nbp.size(); ++d )
        n *= nbp[ d ];
    // std::vector<TF> tmp_res( n * thread_pool.nb_threads(), 0 );
