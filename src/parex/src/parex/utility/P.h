...
#ifndef P
    #define P( ... ) \
        parex::___my_print( std::cout, #__VA_ARGS__ " -> " , __VA_ARGS__ )
    #define PI( N, ... ) \
        parex::___my_print( std::cout, N " -> "            , __VA_ARGS__ )
    #define PE( ... ) \
        parex::___my_print( std::cerr, #__VA_ARGS__ " -> " , __VA_ARGS__ )
    #define PN( ... ) \
        parex::___my_print( std::cout, #__VA_ARGS__ " ->\n", __VA_ARGS__ )
    // PRINT with file and line info
    #define PM( ... ) \
        parex::___my_print( std::cout, #__VA_ARGS__ " -> ", __VA_ARGS__, parex::WithSep{""}, " (", __FILE__, ':', __LINE__, ')' )
    // PRINT with counter
