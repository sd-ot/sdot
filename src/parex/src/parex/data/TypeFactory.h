...
public:
    ...
    Type*         reg_template  ( const std::string &name, TemplateFunc &&f ); ///< f is stored to be called to construct the type if not already created
    ...
    Type*         reg_type      ( const std::string &name, const std::function<void( Type * )> &f ); ///< f is called to construct the type if not already created
