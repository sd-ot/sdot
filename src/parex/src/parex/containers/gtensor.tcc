...
void gtensor<T,N,A>::_set_at( I index, const T &value ) {
    copy_memory_values( *_allocator, _data + index, *reinterpret_cast<BasicCpuAllocator *>( 0ul ), &value, 1 );
}

template<class T,int N,class A>
T gtensor<T,N,A>::_get_at( I index ) const {
    T value;
    copy_memory_values( *reinterpret_cast<BasicCpuAllocator *>( 0ul ), &value, *_allocator, _data + index, 1 );
    return value;
}
